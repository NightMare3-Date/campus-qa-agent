...
class CampusRAG:
    """基于 Chroma + bge-small-zh-v1.5 的 RAG 引擎"""

    def __init__(self, knowledge_dir: Optional[str] = None):
        self.knowledge_dir = str(knowledge_dir or KNOWLEDGE_DIR)
    ...
    def build_index(self) -> int:
        ...
        if emb is None:
            # 模型加载失败，尝试已有索引
            if os.path.isdir(CHROMA_DIR) and self.load_existing():
                cnt = self.db._collection.count() if self.db is not None else 0
                return cnt
            raise RuntimeError(
    ...
    def load_existing(self) -> bool:
        ...
        try:
            ...
            # 通过 _collection.count() 快速验证索引是否有效
            _ = self.db._collection.count()
            return True
        except Exception as e:
